...
```json
{
  "summary_metric": 0.999735736883681
}
```
...
The current baseline (A planted rogue / off-line zero produces detectable amplification under the tested compression and gauge settings) was not confirmed on this run (primary metric rogue_amplification_signature: observed ≈ 0.999736). A revised lemma should account for the observed deviation, possibly via one of the alternative hypotheses below.

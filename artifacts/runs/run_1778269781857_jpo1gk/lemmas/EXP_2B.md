...
The current baseline (A single-perturbed-zero model predicts residual behavior according to the current residual-ratio model (observed/predicted residual ~ 1.0 under the tested envelope)) was not confirmed on this run (primary metric observed_over_predicted_residual_ratio: observed ≈ 9.68867). A revised lemma should account for the observed deviation, possibly via one of the alternative hypotheses below.
